"""pytest plugin: fixtures simulating a JMRI server for jmri-core/jmri-cli/jmri-mcp tests.

Registered via the `pytest11` entry point in jmri-core's pyproject.toml
(test extra), so any of the three packages' test suites gets these
fixtures just by having jmri-core[test] installed — no explicit
`pytest_plugins = [...]` needed in each package's own conftest.py.
"""
