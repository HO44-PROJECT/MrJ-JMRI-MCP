...
def test_lookup_joins_available_list_under_cap():
    message = lookup(
        "en", "errors.unknown_entity", kind="turnout", query="tgv", available=["A", "B", "C"]
    )
...
def test_lookup_truncates_available_list_over_cap():
    ...
    message = lookup(
        "en", "errors.unknown_entity", kind="turnout", query="tgv", available=available
    )
...
def test_lookup_truncates_matches_list_over_cap():
    ...
    message = lookup(
        "en", "errors.ambiguous_entity", kind="turnout", query="layout", matches=matches
    )
    shown = ", ".join(f"T{i}" for i in range(15))
